...
__all__ = ['C3k2_EfficientViMBlock', 'C3k2_EfficientViMBlock_CGLU']
...
class HSMSSD(nn.Module):
    ...
    def forward(self, x, size):
        ...
        h = self.out_proj(h.clone() * self.act(z.clone()) + h.clone() * self.D)
...
class C3k_EfficientViMBlock(C3):
...
class C3k2_EfficientViMBlock(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k_EfficientViMBlock(self.c, self.c, 2, shortcut, g) if c3k else EfficientViMBlock(self.c) for _ in
            range(n)
        )
 
class C3k_EfficientViMBlock_CGLU(C3):
    """C3k is a CSP bottleneck module with customizable kernel sizes for feature extraction in neural networks."""
...
class C3k2_EfficientViMBlock_CGLU(C2f):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k_EfficientViMBlock_CGLU(self.c, self.c, 2, shortcut, g) if c3k else EfficientViMBlock_CGLU(self.c) for _ in
            range(n)
        )